...
contract FusioFantasyGameV2 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    // Admin fee settings
    uint256 public constant MAX_ADMIN_FEE_PERCENTAGE = 50;
    ...
    mapping(uint256 => uint256) public gameEntryCount;        // gameId => number of portfolios created
    ...
    event PortfolioCreated(uint256 indexed gameId, uint256 indexed portfolioId,uint256 gameEntryCount,uint256 prizePool, address indexed owner);
    ...
    function initialize(address _usdcToken, address _adminWallet) public initializer {
    ...
    /// @param gameId Unique identifier for the game
    function updateGameStatus(uint256 gameId) external onlyRole(GAME_CREATOR_ROLE) {
    ...
    function createPortfolio(uint256 gameId, uint256 portfolioId) external nonReentrant {
        ...
        require(gameEntryCount[gameId] < game.entryCap, "Entry cap reached for this game");
        ...
        emit PortfolioCreated(gameId, portfolioId, gameEntryCount[gameId], game.totalPrizePool, msg.sender);
    ...
    function assignReward(
        ...
        require(hasRole(DEFAULT_ADMIN_ROLE, signer), "Invalid signature");
        unchecked { nonce++; }
    ...
    function batchAssignRewards(
        ...
        require(portfolioIds.length == amounts.length, "Mismatched input lengths");
    ...
    function setAdminFeePercentage(uint256 newPercentage) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newPercentage <= MAX_ADMIN_FEE_PERCENTAGE, "Admin fee cannot exceed 50%");
